...
class Predictor:
    ...
    def __init__(self, model, vt_model, _):
        self.clf = model
        self.tfidf_vect = vt_model
    ...
    def predict(self, message: str) -> Prediction:
        ...
        predicted_class: str = self.clf.predict(transformed)[0]
        raw_scores = self.clf._predict_proba_lr(transformed)
        scores = raw_scores[0]
        labeled_scores = dict(zip(self.clf.classes_, scores.tolist()))
